...
from .multimodal_projector.builder import build_denoiser
...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if getattr(self.config, 'diffusion_loss', False):
            ...
            for p in self.pixel_decoder.parameters():
                # In case it is not frozen
                p.requires_grad = False
            if getattr(self, 'denoiser', None) is None:
                self.denoiser = build_denoiser(self.config)
            else:
                # In case it is frozen by LoRA
...
class LlavaMetaForCausalLM(ABC):
    ...
    def compute_vm_loss(
        ...
        img_token_where: torch.Tensor,
    ):
        batch_size = hidden_states.shape[0]
        image_hidden_states = hidden_states[img_token_where].reshape(
            batch_size, 576, -1
        )
        ...
        with torch.amp.autocast('cuda', dtype=torch.float32):
            image_hidden_states = self.model.denoiser.ln_pre(image_hidden_states)
